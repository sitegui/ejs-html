...
module.exports.prepareContent = function (element, compileDebug) {
	...
	contents.forEach((tokens, name) => {
		let content = compile.createCode(reduce(tokens, compileDebug), compileDebug, true)
		jsCode += `\n\t\t"${jsEscape(name)}": ${content},`
	})
...
module.exports.preparePlaceholder = function (element, compileDebug) {
	let name = getNameAttributeValue(element),
		...
		content = compile.createCode(reduce(element.children, compileDebug), compileDebug, true)
	return {
		type: 'ejs-raw',
		start: element.start,
		end: element.end,
