...
module.exports = function (tokens, options) {
	...
	function appendText(str, isPlainText) {
		...
		if (isPlainText && options.collapseText) {
	...
	function appendAttributes(attributes, selfClose, collapse) {
		...
		for (let i = 0, len = attributes.length; i < len; i++) {
			...
			if (attribute.type === 'attribute-simple') {
				...
				if (collapse && attribute.name === 'class') {
					value = value.trim().replace(/\s+/g, ' ')
				} else if (options.boolAttribute &&
					value &&
					booleanRegex.test(attribute.name)) {
