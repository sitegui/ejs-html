...
let nonTextStartRegex = /<(!DOCTYPE |!--|%=|%-|%(?!%)|\/|(?!%%))/g,
	...
	voidElementsRegex = /^(area|base|br|col|embed|hr|img|input|keygen|link|menuitem|meta|param|source|track|wbr)$/i,
 ...
 * @property {?boolean} selfClose - present for type: element
...
module.exports = function parse(source) {
	...
	while (true) {
		...
		if (match[1] === '!DOCTYPE ') {
			...
			if (!openTag.selfClose && !openTag.isVoid) {
